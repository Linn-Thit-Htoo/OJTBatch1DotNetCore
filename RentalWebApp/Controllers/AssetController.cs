...
public class AssetController : Controller
{
    public IActionResult AssetManagement()
    {
        try
        {
            ...
            string query =
                @"SELECT AssetId, AssetCode, Category.CategoryName, Quantity, AssetName, AssetStatus, CreateDate, Asset.IsActive
FROM Asset
...
WHERE Asset.IsActive = @IsActive AND Category.IsActive = @IsActive";
            DataTable dt = DbHelper.Query(
                query,
                sqlParameters: new SqlParameter("@IsActive", true)
            );

            string jsonStr = JsonConvert.SerializeObject(dt);
            List<AssetResponseModel> lst = JsonConvert.DeserializeObject<List<AssetResponseModel>>(
                jsonStr
            )!;
  ...
  FROM [dbo].[Asset] WHERE AssetCode = @AssetCode AND IsActive = @IsActive";
            List<SqlParameter> sqlParameters =
                new() { new("@AssetCode", dataModel.AssetCode), new("@IsActive", true) };
            ...
            string query =
                @"INSERT INTO Asset (AssetCode, CategoryId, AssetName, AssetStatus, Quantity, CreateDate, IsActive)
VALUES (@AssetCode, @CategoryId, @AssetName, @AssetStatus, @Quantity, @CreateDate, @IsActive)";
  ...
  FROM [dbo].[Asset] WHERE AssetId = @AssetId AND IsActive = @IsActive";
            List<SqlParameter> parameters = new() { new("@AssetId", id), new("@IsActive", true) };
  ...
  FROM [dbo].[Category] WHERE IsActive = @IsActive";
            ...
            List<CategoryDataModel> categories = JsonConvert.DeserializeObject<
                List<CategoryDataModel>
            >(categoryJson)!;

            EditAssetResponseModel respModel =
                new() { Categories = categories, AssetDataModel = assetDataModel };
  ...
  FROM [dbo].[Asset] WHERE AssetCode = @AssetCode AND IsActive = @IsActive AND AssetId != @AssetId";
            ...
            string query =
                @"UPDATE Asset SET CategoryId = @CategoryId, AssetCode = @AssetCode, AssetName = @AssetName,
AssetStatus = @AssetStatus, Quantity = @Quantity WHERE AssetId = @AssetId";
    ...
    public IActionResult Delete(long id)
    {
        try
        {
            ...
            List<SqlParameter> parameters = new() { new("@AssetId", id), new("@IsActive", false) };
            int result = DbHelper.Execute(query, parameters.ToArray());
